...
{
    printf("||%s(%p)-%c(%02X)||", ttypename(novariant(o->tt)), (void*)o,
...
static int lua_checkpc(lua_State* L, CallInfo* ci)
{
    ...
    else
    {
        ...
        StkId f = (L->status != LUA_YIELD || ci != L->ci) ? ci->func : restorestack(L, ci->extra);
...
static char* buildop(Proto* p, int pc, char* buff)
{
    ...
    switch (getOpMode(o))
    {
        ...
        case iAx: sprintf(buff + strlen(buff), "%-12s%4d", name, GETARG_Ax(i)); break;
    }
    return buff;
}
...
static int gc_color(lua_State* L)
{
    ...
    else
    {
        ...
        lua_pushstring(L, isdead(G(L), obj) ? "dead" : iswhite(obj) ? "white" : isblack(obj) ? "black" : "grey");
    }
    return 1;
}
...
{
    static const char* statenames[] = { "propagate",    "atomic",   "sweepallgc", "sweepfinobj",
                                        "sweeptobefnz", "sweepend", "pause",      "" };
    static const int states[] = { GCSpropagate,  GCSatomic, GCSswpallgc, GCSswpfinobj,
...
{
    static const luaL_Reg libs[] = { { "_G", luaopen_base },
...
{
    static const char* const codes[] = { "OK", "YIELD", "ERRRUN", "ERRSYNTAX", "ERRMEM", "ERRGCMM", "ERRERR" };
